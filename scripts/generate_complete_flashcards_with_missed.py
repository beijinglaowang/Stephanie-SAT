...
SITE_DATA_DIR = os.path.join(ROOT, "Stephanie-SAT", "data")

MASTER_CSV = os.path.join(OUTPUT_DIR, "vocabulary_flashcards_duplex_10cards_full_a4_source.csv")
MISSED_CSV = os.path.join(OUTPUT_DIR, "missed_10_flashcards_duplex_a4_source.csv")
COMPLETE_CSV = os.path.join(OUTPUT_DIR, "vocabulary_flashcards_complete_deduped_source.csv")
COMPLETE_PDF = os.path.join(OUTPUT_DIR, "vocabulary_flashcards_complete_deduped.pdf")
...
def merge_entries(master: list[Entry], missed: list[Entry]) -> tuple[list[Entry], int, int]:
...
def main() -> None:
    ...
    master = read_entries(MASTER_CSV)
    missed = read_entries(MISSED_CSV)
    entries, added_count, duplicate_count = merge_entries(master, missed)
    ...
    print(f"Missed rows: {len(missed)}")
